...
from fastapi import FastAPI, Request, WebSocket, WebSocketDisconnect, File, UploadFile, BackgroundTasks, HTTPException, Depends, Query
...
@app.get("/api/reports")
async def get_all_reports(report_ids: str = None, _email: str = Depends(get_current_user_email)):
    report_ids_list = report_ids.split(",") if report_ids else None
    reports = await report_store.list_reports(_email, report_ids_list)
    return {"reports": reports}


@app.get("/api/reports/{research_id}")
async def get_report_by_id(research_id: str, _email: str = Depends(get_current_user_email)):
    report = await report_store.get_report(research_id, _email)
    if report is None:
...
async def create_or_update_report(request: Request, _email: str = Depends(get_current_user_email)):
    try:
        data = await request.json()
        research_id = data.get("id", "temp_id")

        now_ms = int(time.time() * 1000)
        existing = await report_store.get_report(research_id, _email)
        ...
        await report_store.upsert_report(research_id, report, _email)
...
async def update_report(research_id: str, request: Request, _email: str = Depends(get_current_user_email)):
    existing = await report_store.get_report(research_id, _email)
    if existing is None:
        raise HTTPException(status_code=404, detail="Report not found")
    ...
    await report_store.upsert_report(research_id, updated, _email)
    return {"success": True, "id": research_id}


@app.delete("/api/reports/{research_id}")
async def delete_report(research_id: str, _email: str = Depends(get_current_user_email)):
    existed = await report_store.delete_report(research_id, _email)
    if not existed:
        raise HTTPException(status_code=404, detail="Report not found")
    return {"success": True}
...
async def get_report_chat(research_id: str, _email: str = Depends(get_current_user_email)):
    report = await report_store.get_report(research_id, _email)
    if report is None:
        raise HTTPException(status_code=404, detail="Report not found")
...
async def add_report_chat_message(research_id: str, request: Request, _email: str = Depends(get_current_user_email)):
    report = await report_store.get_report(research_id, _email)
    if report is None:
        raise HTTPException(status_code=404, detail="Report not found")
...
@app.get("/files/")
async def list_files(_email: str = Depends(get_current_user_email)):
...
@app.post("/upload/")
async def upload_file(file: UploadFile = File(...), _email: str = Depends(get_current_user_email)):
    return await handle_file_upload(file, DOC_PATH)


@app.delete("/files/{filename}")
async def delete_file(filename: str, _email: str = Depends(get_current_user_email)):
...
@app.post("/api/chat")
async def chat(chat_request: ChatRequest, _email: str = Depends(get_current_user_email)):
