          ...
          <div className="flex-1 ai-message-bubble rounded-2xl px-4 py-3 text-sm text-gray-900 mr-4 shadow-lg">
            ...
            {metadata && 
             ...
             metadata.sources.length > 0 && (
              <div className="mt-2 pt-2 border-t border-gray-200/50 text-xs text-gray-700">
                <details className="group">
                  <summary className="cursor-pointer hover:text-gray-900 flex items-center">
                ...
                {allSources.map((source: {name: string; url: string}, index: number) => {
                  ...
                  return (
                    <a 
                      ...
                      target="_blank" 
                      rel="noopener noreferrer" 
                      className="inline-flex items-center gap-1 px-1.5 py-0.5 text-xs bg-gray-50/60 text-gray-600 hover:text-teal-600 hover:bg-gray-50/90 rounded border border-gray-200/40 transition-colors"
                      title={source.name}
                    >
...
const MobileChatPanel: React.FC<MobileChatPanelProps> = ({
  ...
  return (
    <div className={cn("flex flex-col h-full bg-gradient-to-b from-white to-gray-50", className)}>
      ...
      {/* Input Area */}
      <div className={`px-3 py-3 border-t border-gray-200 ${inputFocused ? 'bg-gray-50/90' : 'bg-white/90'} backdrop-blur-sm transition-colors duration-200 safe-bottom`}>
        {!isStopped ? (
          <form onSubmit={handleSubmit} className="relative">
            <textarea
              ...
              className="w-full px-4 py-3 pr-14 bg-gray-50/90 border border-gray-200 focus:border-teal-500 rounded-xl resize-none text-gray-900 text-sm placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-teal-500/50 transition-all shadow-sm"
          ...
          <div className="text-center p-3 text-gray-600 bg-gray-50/60 rounded-xl border border-gray-200/50 text-sm shadow-sm">
            Research has been stopped. 
            {onNewResearch && (
              <button 
                onClick={onNewResearch} 
                className="ml-2 text-teal-600 hover:text-teal-600 hover:underline font-medium"
              >
                Start new research
              </button>
            )}
