...
- **知识库**：`/knowledge`沿用248px导航、深色圆角主画布和紧凑字号，使用资料库列表+文档详情，不改成白色表单页或装饰卡片。创建/编辑在详情区完成，索引状态、更新时间、原文与删除为行级操作；窄屏重排，不隐藏返回任务入口。输入区“资料”打开小型范围选择器，提供自动/指定/关闭三种语义；指定库是显式问答上下文，不能暗中改变成联网研究。报告入库显示提交索引状态，不将提交成功写成已可检索。
- **列表与筛选**：标题、来源切换、搜索、状态过滤为一个紧凑工具区。列表优先名称、状态与时间；ID 为次级信息。加载、空数据、无搜索结果、接口错误分别呈现。
- **记忆编辑**：放在Agent现有“记忆”面板，不另建一套工作台。范围选择、紧凑文件切换、等宽Markdown正文与保存/读盘操作按顺序排列，路径可换行。文件冲突展示磁盘内容并保留草稿，不能用刷新丢弃用户修改；未保存时禁用切换和关闭，保留明确的读盘/放弃草稿操作。正文不渲染可执行HTML。
