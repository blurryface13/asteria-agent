...
def test_routes_deliverable_without_review_keyword():
    async def model(system, user):
        assert json.loads(user) == {'message':'比较几篇论文的方法与局限，写相关工作','history':None,'report':'','knowledge_catalog':[]}
...
def test_routes_self_contained_request_to_general_chat():
    async def model(system, user):
        assert json.loads(user) == {'message':'把这句话改得更自然','history':None,'report':'','knowledge_catalog':[]}
