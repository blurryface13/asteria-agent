...
### 2026-09-22：EchoMind式工具契约与Coding证据门槛

- 对照本机EchoMind的BaseAgent._call_llm、角色契约与共享RAG工具，适配调用ID、调用成功/业务成功区分及耗时Trace。保留原模型接口与工具白名单，不照搬客服3轮上限，不将源码中不存在的逐条证据校验当作已复用功能。
- 生产Coding循环限制同参数只读/诊断最多两次执行；明确论文/知识库依赖的任务须有实际片段才能completed，支持直接阅读或携证据的科研回复。摘要本身不算证据，失败文件结果不允许用于静态诊断；当前门槛不等于论断级语义验证。
- 新增12条回归；全量219通过/5环境跳过。真实模型去掉实验专用强制路由提示后，两路径均取得证据并完成建议：74a39ac6d3，直接4次/9.209秒，委派7次/20.692秒。未执行生成代码、未应用修改，不用单例作为普遍性能指标。
- 实现、EchoMind对应关系与限制见 docs/coding-tool-contract.md。保留旧失败记录与之前登录/公共知识库未提交改动。

### 2026-09-22：Coding直接阅读与科研子循环对照

- 新增受控配对实验：同一只读代码夹具、人工转述冻结论文材料、模型与预算，分别开放直接论文工具或request_research；复用生产Coding循环与真实科研子循环，不启动完整报告流程。默认线上策略不变，没有执行代码或修改用户文件。
- 真实deepseek-chat服务本次可用。有效配对8f4bc0ed69：直接组4次调用/9.593秒/9,884总Token，委派组7次/16.511秒/19,507总Token，求助等待5.019秒；两组取得实际片段、生成相同diff。本结果只有单例，且是修改建议，不能称为代码修复成功率或通用加速比例。
- 首轮bc6d245cda保留为BadCase：委派组6次重复读文件，未读论文、未求助却自报完成。补充实验路径指令和独立证据/交接验收，新增回归反例；不宣称生产完成校验已全面修复。
- 新增8条确定性回归；当前工作区全量207通过/5跳过。实验方法、局限、复现命令与后续隔离执行器要求见 docs/coding-research-comparison.md。仅提交本轮文件，不混入之前的登录/公共库改动，不重启运行中服务。

### 2026-09-22：EchoMind 风格的入口融合与普通角色分流

- 新增 intent_fusion.py：LLM Few-shot / Embedding / 规则三路融合，其中LLM与向量分支并发，接现有 Ollama bge-m3；字符降级单独标注并降低权重，不把 n-gram 当成神经向量。LLM错误不退回关键词继续执行，冲突或低置信度先澄清。
- 真正的LRU＋TTL缓存按用户、会话、完整消息/历史/报告、可访问知识库、记忆快照和模型配置隔离；静态模板向量共享并合并冷启动编码。配置变更、记忆修改、身份变化均不能复用旧判定。
- Coordinator 保留科研 durable Run 与普通角色的分流；代码角色共用研究内部的coding循环。旧研究入口及外部LangGraph前端入口均加非研究/澄清保护，避免投稿、代码等请求误启动研究流程。当前注册10类能力，不能继续将旧简历“8类”作为实现事实。
- 全量后端199通过/5环境跳过，入口专项33条；真实PG/Redis指定集14条通过，前端8条与TypeScript通过。全量和专项有重叠，不累加。自动化模型决策是替身，不代表真实分类质量。
- 另将待提交文件导出至独立临时副本，排除尚未提交的登录/共享库实现；加载本机数据库测试配置后全量195通过/4跳过，Coordinator与durable Run真实PG集9条通过。第一次副本全测缺DATABASE_URL导致2项环境失败，补充测试环境后通过，未修改业务代码掩盖失败。
- 六条向量探针真实调用bge-m3且均匹配预期；记录 outputs/intent-routing-aa210d7c51/summary.json。向量相似度不等于识别准确率；此前真实LLM验收被HTTP402阻断，未换模型或写入虚构效果数据。
- 协作核心已提交 b5b02d6；本增量保留以前的登录/共享知识库改动，不混入这次路由提交。可复现实验与当前限制见 docs/agentic-collaboration-acceptance.md。
- 确认研究/Coordinator/索引活动均为0后，重载8018 API与研究Worker。健康接口ok/shared_mode=true，未登录能力与文件接口401，3023/login返回200；前端不重启，不清构建缓存。

### 2026-09-22：代码Loop直接论文工具、静态诊断与阶段结果回传

- 同一coding循环可自行选择读论文或有界request_research，不强制委派。直接读论文复用任务内PaperLibrary、原文页段与证据预算。基于实际文件observation_id进行Python语法检查和diff预览，不执行代码、不应用修改。
- 并行分工改为as_completed逐项保存并推送结果，保留失败/取消和首项回传/整批耗时；Lead仍在本批回传后验收，不声称已做通用DAG调度。求助回到原代码Loop，记录等待耗时。
- 按impeccable沿用原深色UI，增加紧凑分工列表、阶段结果、定向求助状态和可展开详情；明确“阶段结果未验收”。Coordinator保存按会话鉴权的有界progress，页面800ms轮询与刷新恢复。
- 验证：协作专项35条、全量后端166条通过/5条环境跳过；真实PG/Redis指定集14条通过；前端TypeScript和8条组件回归通过。隔离组件浏览器验收通过，不连接真实用户/模型，样例时间不作指标。
- 保留HTTP402真实模型阻断记录；本轮不宣称分工质量或代码建议已通过真实模型评测。验收指令和源码对照更新到docs/agentic-collaboration-acceptance.md。

### 2026-09-22：Lead 分工质量与 Coding 定向调研协作

- 增加角色契约、目标归属矩阵、派发前独立语义审查；拒绝重复/越界/遗漏分工，将失败观察返回 Lead 修订。复用现有自主行动循环和并行共享证据，不改成固定步骤图。
- 代码角色共用有限工具循环，接只读 GitHub 调查、当前用户 MCP 文件工具、request_research。求助要求先读代码，携带具体疑问/观察/期望答案，研究子任务返回原代码循环；限制两次求助、全局子 Agent 预算和取消传播。
- 保留独立论文审查，新增代码交付独立验收。用户明确提出的代码/实验目标单列 c-ID；待批准提案不能当已应用修改，未接实验执行器不能通过执行验收。
- 156条全量后端测试通过、5条环境条件测试跳过；另外开启真实PG/Redis集成的指定测试集14条通过。新增25条协作测试含真实MCP子进程/隔离文件读取与脚本模型求助链路。测试范围、命令及区别见 docs/agentic-collaboration-acceptance.md。
- 真实GitHub工具成功列出 psf/requests 的127个普通文件，并按固定commit读取 docs/_themes/flask_theme_support.py，4875字符；不下载运行代码。
- 真实LLM验收第一步被当前模型服务HTTP402余额不足阻断，SDK尝试3次。未完成分工质量/建议质量验证，未换模型，不计为通过；记录 outputs/collaboration-eval-0ce962d515/summary.json。
- 重载前确认研究、Coordinator、知识库索引活动数均0；重载8018后端和研究Worker，/health为ok/shared_mode，未登录能力接口401，3023/login为200。未修改前端、用户历史数据或模型配置。上轮共享版未提交修改保持原状。

### 2026-09-18：实验室共享版测试前闭环（Codex / GPT-5，进行中）

- 原始需求、EchoMind参考、简历差异、分阶段计划和验收边界先记录在 `docs/lab-release-plan.md`。用户确认管理员预置账号＋密码、历史归属迁移邮箱；不重做科研核心、不将占位指标当实测。
- 后续每阶段在计划实施记录中补充测试与真实运行，再commit。当前是计划节点，不是上线完成声明。

### 2026-09-13：重命名与任务移动（Codex / GPT-5）

- 项目/任务重命名、任务跨项目移动及解除归属已实现。复用EntityActions菜单内编辑，不新增大弹窗；按impeccable规范保留原深色外壳与键盘交互。搜索改为工作区对话+旧报告并集，名称以数据库标题为准。
- 服务端移动事务校验归属和活动状态，名称拒绝空白；不重写消息、报告或文件路径。dora隔离PostgreSQL与请求模型测试3项通过，原前端测试2项通过，TypeScript检查通过。
- 真实页面依次将“管理验证A”改名、将任务改名、从A移到B，刷新确认项目树，再移出项目。API复核名称和project_id以及原消息保留。测试对象：项目cbe776fe-77b2-4adc-8d90-fdf360b7d683、0f6a0ed9-40b7-4eb1-9724-1a35c0fc4b8c，对话3a728ed5-a492-4b75-b61c-b67c5d04a036。验证后仅清理这三个临时对象，不影响用户历史。
- 新移动接口需要API重载，重载前确认研究和Coordinator活动数均为0，仅重载8018；前端和worker未关闭。不改依赖版本、构建缓存、代理或证书验证。
- 后续顺序按用户最新要求调整为知识库管理/主Chat闭环→分层记忆→受控实验/评测，记忆设计见spec §23，本阶段不宣称长期记忆已实现。

最新开发节点：2026-09-22 入口融合、分工审查与代码调研路径对照，见本文顶部。当前仓库为 `/Users/dora/Developer/asteria-agent`；模型服务已恢复并完成局部真实对照，完整分工质量与执行调试仍未验收。下一执行能力需另设隔离与授权边界，不以历史待办覆盖最新状态。
...
- 当前 Agentic 边界：研究视角、工具动作和研究策略重规划由 Agent 根据证据自主选择，子 Agent 可并行执行；已实现分阶段 Skill 发现、按需注入及用户指定。工具预算、证据来源和交付校验由运行器约束；远程自主实验执行尚未完成。既有离线 RAG 独立保留，不等于已完成在线研究成果的全量自动入库。
- 当前开发路线以 [自主实验与评测路线](docs/experiment-evaluation-roadmap.md) 为准；下文保留历次节点，不以历史待办覆盖最新实现状态。

### 2026-09-12：Agentic 能力闭环自查（GPT-5）

- 对照当前实现和历史真实运行记录复核了文献综述主链路：入口由自然语言意图识别决定能力；Planner 生成并经过契约校验的研究目标、过程要求、交付约束和可选扩展；用户确认后，lead 每轮从结构化动作中选择下一步，不按固定章节 DAG 执行。
- 研究动作覆盖检索、原文阅读、页段分页、参考文献追踪、已有证据检索、按未完成目标委派、按需加载 Skill 和结束。工具失败只作为观察返回，不转换成空证据或静默切换模式；论文下载、原文证据、引用边和报告来源均由运行器校验并落盘。
- lead 的 `delegate` 会把一个到三个目标绑定给独立研究子 Agent，并通过 `asyncio.gather` 批次并行；每个子 Agent 有独立观察历史和动作循环，不能递归委派或请求用户，结果回到共享证据目录后再由 lead/assessor 结算。新增回归测试锁定“主 Agent 实际委派并合并共享证据”，不是只测试孤立子 Agent 并行；dora 全量回归为 106 passed。
- 充分性审查使用独立上下文，只接收确认目标、实际读取的原文片段和已验证引用关系；`direct / synthesis / inference / unknown` 区分作者事实、跨来源综合、限定推断和未知。审查只决定核心证据是否足够；可选扩展、篇幅和格式不能制造补研循环。通过后才进入 Writer、受控 LaTeX/PDF 发布，引用只能来自已读取来源。
- 真实窄范围任务 Run `3726b79dd71c4a2ba73bcd9fd2d1d683` 已完整经过规划确认、原文阅读、审查重检、写作 Skill、引用图、报告检查和 PDF 发布。该任务显示 0 个研究子任务是“单篇指定原文”下的合理直接路径；开放域综述仍应由 lead 根据目标缺口自主决定是否委派，不能用子任务数量作为唯一 Agentic 指标。
- 当前判断：项目已经达到“有边界的自主科研 Agent”阶段，已超越旧的固定研究视角/章节工作流；尚不是无限制自主系统。未完成边界包括持久后台子 Agent、跨进程 checkpoint 恢复、任意社区 Skill 自动安装/执行、实验实际执行和全面的论断级质量评测。后续新增功能必须保持这些边界显式可见，不能把固定兜底流程包装成自主能力。
- 本轮新增受确认契约保护的运行中重规划动作、回归测试和开发记录，没有修改前端视觉、数据库、模型配置或服务启动方式；3023 前端和 8018 后端继续保持运行并返回 HTTP 200。重规划上限为 `REVIEW_MAX_PLAN_REVISIONS=3`（服务端可配置），不改变研究目标、过程要求和交付约束。
...
### 2026-09-13：项目/任务删除与头像调整（Codex / GPT-5）

- 真实访问参考产品chat页并展开任务菜单，核对省略号入口、任务设置、置顶、重命名、隐藏与删除的组织方式，仅查看不操作对方数据。本轮按项目既有设计语言落地删除，不复制无实现的菜单项目。
- 新增EntityActions复用组件，项目、项目内对话及独立任务都有行尾菜单。菜单Portal独立于侧栏滚动层，支持方向键、Home/End、Escape、键盘焦点与触屏；删除确认默认聚焦取消，包含名称、范围、忙碌和失败状态。
- 复用现有后端删除接口，不改数据库Schema：项目删除保留子任务并解除project_id；任务删除沿用服务端归属/活动检查和事务清理。本地文件保留。删除当前任务同时重置待用会话ID和输入内容。
- 删除旧缓存自动上传逻辑，避免已删除报告被另一个旧浏览器再次创建；原缓存独立备份，服务端列表为准。不提供自动恢复或隐藏的重新导入。
- 用户提供的PNG原样加入public/img/agent-avatar.png，替换通用/学术/混合检索选择器图标和任务研究身份行图标；不改左上角猫标识及导航图标。DESIGN.md记录尺寸和例外，不修改整体颜色、字号、布局或Agent协议。
- 验证：Node组件/Hook测试2项通过，覆盖确认前不发送删除、失败保留、成功关闭、禁用/键盘行为和旧缓存不回传；dora隔离PostgreSQL测试1项通过，覆盖归属、子任务保留、运行中Chat/Research阻止删除和终态记录清理；TypeScript无增量检查通过。
- 真实页面验证临时项目菜单、确认范围、取消以及两处头像；通过API删除本轮临时项目后，浏览器刷新确认子任务进入独立列表。随后API删除临时任务，GET404且列表不再包含。临时对象为项目8792b96b-9401-49dc-b4bd-40dde271403b、对话06ea34b6-08ac-4405-9c3a-ec9397e57bb9，不删除用户历史数据。最终删除回调由组件测试与真实接口分层验证，未把浏览器打开确认弹窗等同于完整点击删除验收。
- 本轮没有新模型调用，没有停止/重启前后端和worker，没有升级依赖或改构建目录。参考产品对比与impeccable产品规范主要影响菜单收纳、焦点可达性和删除边界提示。

### 2026-09-13：知识库管理、Coordinator 选库与版本问答（Codex / GPT-5）

#### 开发节点

- 承接 `a3c24d5` 的项目/任务重命名和移动，本轮不改分组行为。分层记忆保留为spec §23.1的下一步，先完成个人知识库；没有把知识库检索宣传为长期记忆。
- PostgreSQL新增资料库、稳定文档、版本、片段、向量清理队列。实现新建/编辑、PDF/Markdown/TXT上传、SHA256去重、后台增量索引、原文下载、同名文档版本切换、删除与报告显式入库。更新失败保留旧发布版本并显示失败；删除不等待Chroma清理即可阻止新的召回，物理向量清理由持久化GC完成。
- 主Chat新增自动发现、指定知识库问答和关闭检索。Coordinator按可见目录选择最多3个库并复核owner；指定模式是用户明确要求，自动模式由模型判断。资料回答复用BM25/Dense/RRF/精排，来源页码与版本可见，完整证据快照及usage落库。没有改研究Planner、并行子Agent、充分性审查、Skill注入或首页宽度路由。
- 独立知识库页和输入区范围选择延续既有深色规范。impeccable规范影响的是平面列表、紧凑操作和真实状态，不重做研究工作台。报告入库复用上传和版本发布路径；本轮该接口通过隔离数据库测试，未将其描述为真实浏览器报告入库验收。

#### 环境与真实发现

- 外部RAG引擎旧位置仍受iCloud影响，settings和Chroma文件有dataless，status502。恢复同一上游提交到 `/Users/dora/Developer/modular-rag-engine`，复制原data约1.4GB，rsync大小/mtime复核无差异。旧源码工作树状态读取受阻，不能证明未知本地补丁已迁移；旧目录完整保留。
- dora缺少chromadb和jieba。对照原环境安装Chroma1.5.9并锁定，补齐jieba0.42.1及缺失依赖；没有切换虚拟环境或升级已有包。`pip check`通过。旧research_papers含25,832个chunk，真实hybrid_rerank检索“RAG如何减少幻觉”成功，5.88秒；没有重跑Golden Set或RAGAS。
- 第一次资料问答因 `ModuleNotFoundError: jieba` 失败，turn `5fe2e159-0b40-49ad-97d6-ab9a2b544bde` 保留为真实失败记录。按依赖根因修复，不用关键词检索兜底。又发现模型可能复制历史引用footer，补充写作约束并由应用统一附加版本元数据。
- 浏览器localStorage备份全量历史触发 `QuotaExceededError`，阻碍数据库历史显示。改成先显示服务器列表、只备份本地独有草稿；缓存失败不撤销成功的数据库保存。空间不足的草稿仍留原缓存并提示，不清用户存储、不回传已删除任务。新增满额回归用例；首次测试失败是VM缺少Event，补齐测试环境后通过，并非运行时代码故障。

#### 真实网页问答

浏览器创建“**Aurora 联调资料**”，上传仓库内明确标注虚构的 `tests/fixtures/knowledge/aurora.md`，再上传 `updated/aurora.md`。资料库 `bd2317e5-6fb3-47d7-8b92-4485b3485800`，文档 `708be34a-d253-44ee-be11-194a77e2ab6d`；同名更新没有多建文档，刷新后名称、状态、消息与选库模式保留。

对话 `f2275672-79f9-4b67-8930-45717d45ba67`：

| 场景 | turn | 结果 | 耗时 / 模型tokens |
| --- | --- | --- | --- |
| 指定库查询训练参数 | `c13e73c6-bbfb-42e9-b137-a0869e7b829a` | knowledge_chat，137步、seed23、每7天，引用首版 | 12.543秒 / 600 |
| 切换自动模式询问存储资料 | `0effbb63-cb3b-4517-951f-b0f244c4b1e7` | 模型自主选Aurora库，返回loss/accuracy/elapsed_seconds | 2.935秒 / 1,538 |
| 同名文件更新后追问最新步数 | `ecae0a25-0c1c-4d76-8e80-8040b52f906b` | 重新检索为211步，不沿用历史137；引用新版本 | 4.298秒 / 1,891 |
| 换成Python列表/元组普通问题 | `df51e414-6206-4e6c-a80e-85e3fa35ce8c` | general_chat，无资料引用、无研究Run | 2.229秒 / 1,748 |

该对话合计0个research Run。tokens是provider返回的 `deepseek-chat` 分类/重写/回答合计，不包含embedding/精排成本，不凭别名推断具体DS版本或换算人民币。普通回答对tuple作为字典键的条件表述不够严谨，这次仅用于路由验证，不计作知识准确率通过。

旧版本 `abdab604-c287-423f-8248-f2c0421cad66` 向量数0，新版本 `b279c121-8f9d-4538-b1b2-37aa35fb4e73` 向量数1。另用真实API上传临时虚构文件 `gc-verification-20260913.md`，确认索引可用后删除，向量从1变0；删除对象仅本轮临时文档 `a63315ad-7966-45a3-ad2c-b8704e402396`，未动用户资料。Aurora样例保留便于体验。删除文档会移除其数据库原文，不提供恢复按钮；样例原文件仍可从测试fixture重传。

#### 验证与交付边界

- dora针对性10项通过，覆盖Coordinator、项目归属、知识库版本发布/失败保留/去重、私有collection隔离、报告归属导入、删除级联；隔离数据库测试的embedding/向量使用替身，真实模型、索引和向量清理由上述单独流程验证。1项既有LangChain弃用警告。
- Node组件/Hook测试3项通过，TypeScript无增量检查通过。网页真实上传、显式/自动问答、更新、普通聊天与刷新走通；未重新运行整套付费综述或已有质量benchmark。
- 加载后端修改前检查活动Run、turn和索引队列；只必要重载8018 API，最后一次约12:42。前端3023使用HMR，不重启、不清.next；研究worker未重载，原dora/Node22和端口不变。最后核对知识库页面视觉，未改变原研究页面风格。
...
- 根据用户对本地可编辑分层记忆的确认，替换此前MemoryEntry双存储设想：用户profile/preferences.md、项目.asteria/PROJECT.md和memory/MEMORY.md/主题文件为唯一可编辑正文，PostgreSQL复用owner/project/conversation关系，不增记忆正文表。根目录默认为Developer/asteria-workspaces，与源码和云同步目录分离。
- 新增MemoryService及认证API，支持真实读写、SHA256版本冲突、原子替换、路径/链接限制及.trash恢复目录。页面Agent→记忆提供范围切换、主题新建、Markdown编辑、磁盘比较和删除；沿用impeccable审阅过的既有深色设置风格，不重做工作台。
- 基础记忆默认加载，模型根据问题及目录预览选最多3个主题；Coordinator与durable worker各自读取冻结快照。普通与工具型LLM统一ContextVar注入，子任务继承、结束/异常重置。新建/移动对话由数据库确定项目，不接受前端指定任意磁盘路径。未增加自动写回工具或改变研究/Skill动作空间。
- 网页冲突验证后修正两处交互细节：有草稿时点击当前文件也不能抹掉修改；采用磁盘版后同时更新文件切换列表的版本基线。冲突不会通过清缓存或覆盖磁盘解决。

#### 真实页面与模型记录

浏览器创建测试项目“分层记忆联调” `7d14dbbb-4c66-47e0-ad87-0c9e1641461c`，在Agent面板保存PROJECT和experiments主题；内容明确为虚构测试约定。工作区路径显示在面板中，首次保存完成DB绑定。用户偏好文件没有写入测试内容，避免影响其他项目。

对话 `5bf85e24-a4c7-4fc3-af3b-d4bcd51ef9cc`：

| 场景 | turn | 结果 | 时间 / tokens |
| --- | --- | --- | --- |
| 询问代号、批次和复测间隔 | `0c33cfee-a7e9-4826-a087-e8fffabbc151` | 模型自主选experiments，回答Lyra-47 / 19 / 每9天 | 4.058秒 / 1,686 |
| 本地修改后追问 | `d5efda0d-6a4a-47b3-b06d-45c06e1e1043` | **语义失败**：快照已为37，回答仍为19且否认读取能力 | 3.182秒 / 1,871 |
| 修复注入后同一句追问 | `50a4a348-1629-4437-948a-97cc9cac6c8d` | 回答37 / 每9天，未在问题中提供答案 | 4.317秒 / 2,187 |

三轮均general_chat，0个research Run，合计5,744模型tokens。数据来自provider返回的deepseek-chat usage，包含主题选择、意图分类和回答，不凭别名换算具体模型版本或人民币。失败turn的技术状态completed仅代表回答已返回，不代表语义成功；保留原消息和快照作为BadCase。

根因是原注入位于旧历史之前，且“没有写操作不能声称保存”的说明被模型错误延伸为“没有聊天写入就不能承认磁盘读取”。调整为最新问题前的CURRENT-TURN READ RESULT，明确应用已经读盘、同文件新版本优先于旧历史、外部编辑不要求聊天write事件；仍不将记忆提升为系统权限或论文证据。保留同一对话和原失败回答复测，没有清空历史规避问题。

网页草稿与本地编辑并发测试：磁盘批次先由19改31，重新打开编辑器后保留网页草稿，再本地改37；保存得到409，页面保留31草稿并可查看37磁盘版，明确采用磁盘版后正文与hash更新。文件修改只涉及本轮虚构测试项目。浏览器重开恢复原对话和文件，不依赖localStorage作为正文源。

#### 测试、环境与边界

- dora文件/数据库/Coordinator/知识库/管理/durable Run针对性18项通过，覆盖权限、项目隔离、任务移动、文件版本/大小/链接、可恢复删除、双LLM入口注入一次、worker子任务继承和失败重置。隔离DB使用临时schema，不改用户历史数据；worker研究处理器使用替身，不冒充新完整综述实测。
- Node组件/Hook4项通过，含记忆冲突、草稿保留和切换后新版本；TypeScript无增量检查通过。扩展Agentic回归初测71过、2失败，定位两项旧意图测试仍要求纯字符串，而HEAD实现早已传JSON消息/history/catalog；仅更新测试契约，不为测试修改业务路由，复测73项全过。两组后端测试合计91项，现有LangChain/Swig弃用警告保留。
- 本轮没有安装依赖、清理.next或重启前端。API/worker加载记忆功能和修正注入分别做必要重载，均先确认研究、Coordinator与索引活动数为0；保持dora、Node22、3023/8018及LaunchAgent。前端更新由HMR加载。检查API使用LOCAL_DEV记录的/api/workspace/skills；本项目没有/api/health，访问它404不是服务故障。
- 本轮真实验证限于记忆CRUD、冲突与普通问答，没有重新跑付费完整综述、RAGAS或长期自动记忆效果评测。自动记忆提议/写回、运行中补查、过期清理、远程目录授权仍是后续项；不要将手动维护+自动选读描述为已实现自主记忆写入。spec §23.4、LOCAL_DEV和DESIGN同步维护。
