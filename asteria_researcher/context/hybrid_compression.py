...
class HybridContextCompressor:
    ...
    async def async_get_context(self, query: str, max_results: int = 5, cost_callback=None) -> str:
        ...
        chunk_vecs, query_vec = await asyncio.gather(
            asyncio.to_thread(self.embeddings.embed_documents, texts),
            asyncio.to_thread(self.embeddings.embed_query, query),
        )
        matrix = np.asarray(chunk_vecs, dtype=np.float32)
        q = np.asarray(query_vec, dtype=np.float32)
        cosine = matrix @ q / (np.linalg.norm(matrix, axis=1) * np.linalg.norm(q) + 1e-9)
        ...
        for idx, _score in fused:
            if any(float(matrix[idx] @ matrix[j] /
                         (np.linalg.norm(matrix[idx]) * np.linalg.norm(matrix[j]) + 1e-9))
                   > _DEDUP_COSINE for j in selected):
                continue
            selected.append(idx)
