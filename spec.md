...
参考交互原则：任务页应以持续 transcript 为主，用户输入、Agent 思考、工具动作、文件产物和最终报告都属于同一任务时间线；步骤、子 Agent、主机和用量是辅助检查面板，不应打断主线。当前前端先保留白色液态玻璃主题，但组件层次按“任务导航—执行/报告 transcript—上下文问答—状态检查”重组；后续优先补任务状态时间线、工具调用明细、报告/知识库上下文标识和可恢复的任务输入。
