...
- [x] 首页改为白色研究工作台：左侧项目/任务导航，中间意图入口和研究模板，顶部保留知识库、RAG 工作区与设置入口。
...
视觉上采用深色、低干扰、信息密度适中的工作台风格；卡片用于任务和产物，步骤流用于过程，右侧面板用于可观测信息。具体颜色和组件在实现前再沉淀为前端 tokens。
